          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <select value={entry?.testId || ''} onChange={e => {
                ...
                {tests && Array.isArray(tests) && tests.filter(t => t.subject === selectedSubject).map(t => <option key={t.id} value={t.id} title={t.title}>{t.title.length > 25 ? t.title.slice(0, 25) + '...' : t.title}</option>)}
              </select>
            </div>
          </div>
