...
import {
  ...
  Paperclip, Download, Keyboard, MousePointer2, Loader2
} from 'lucide-react';
import {
            ...
            <div>
              ...
              <div className="relative">
                ...
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                  ...
                  <label className={`p-1.5 rounded-lg transition-colors cursor-pointer ${isUploading ? 'opacity-50 cursor-wait' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`} title="Прикрепить файл">
                  ...
                  {allSlots.map(sl => {
                    ...
                    return (
                      <th key={sl.key} colSpan={totalCols} className={`px-1 py-1 text-center border-b border-r border-gray-300 min-w-[44px] relative ${isToday ? 'bg-green-100' : ''}`}>
                        ...
                        {isToday && (
                          <div className="absolute top-3 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-green-500 rounded-full"></div>
                        )}
                      </th>
                    );
                  })}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {allSlots.map((sl, idx) => {
                ...
                const entry = diaryEntries && Array.isArray(diaryEntries) && diaryEntries.find(e => e.date === sl.date && e.subject === selectedSubject && e.lessonNumber === sl.lessonNumber);
                const lt = getLessonType(sl.date, sl.lessonNumber);
                const ltType = lt ? customLessonTypes.find(c => c.value === lt.type) : null;
                return (
                  <tr key={sl.key} className="border-b border-gray-200 hover:bg-gray-50">
                    <td className="px-3 py-2 text-center text-gray-500">{idx + 1}</td>
                    <td className="px-3 py-2 text-gray-900">
                      {new Date(sl.date + 'T00:00').getDate()} {MONTH_NAMES_GEN[new Date(sl.date + 'T00:00').getMonth()]}
                    </td>
                    <td className="px-3 py-2">
                      {ltType ? <span className={`px-2 py-0.5 rounded text-xs font-medium ${ltType.color}`}>{ltType.short}</span> : <span className="text-gray-400">—</span>}
                    </td>
                    <td className="px-3 py-2 text-gray-900">
                      {entry?.topic || <span className="text-gray-400 italic">Не указана</span>}
                    </td>
                    <td className="px-3 py-2 text-gray-900">
                      {entry?.homework || <span className="text-gray-400 italic">Нет</span>}
                    </td>
                    <td className="px-3 py-2 text-center">
                      {entry?.checkHomework ? <span className="text-green-600">✓</span> : <span className="text-gray-300">—</span>}
                    </td>
                    <td className="px-3 py-2 text-gray-600">
                      {entry?.testId && tests?.find(t => t.id === entry.testId)?.title || <span className="text-gray-400">—</span>}
                    </td>
                  </tr>
                );
              })}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-100 border-b border-gray-300 text-xs text-gray-700">
                <th className="px-3 py-2 text-left w-10">№</th>
                <th className="px-3 py-2 text-left">Ученик</th>
            ...
            <tbody>
              {sortedStudents.map((student, idx) => (
                <tr key={student.id} className="border-b border-gray-200 hover:bg-gray-50">
                  <td className="px-3 py-2 text-center text-gray-500">{idx + 1}</td>
                  <td className="px-3 py-2 font-medium text-gray-900">{student.lastName} {student.firstName}</td>
                  {allDates.slice(-10).map(d => {
                    const att = attendance.find(a => a.studentId === student.id && a.date === d && a.subject === selectedSubject);
                    const at = att ? ATTENDANCE_TYPES.find(a => a.value === att.type) : null;
                    return (
                      <td key={d} className="px-1 py-2 text-center">
                        {at ? (
                          <span className={`inline-block w-7 h-7 leading-7 rounded-md text-xs font-bold ${at.bgColor} ${at.color}`}>
                            {att.type}
                          </span>
                        ) : (
                          <button
                            onClick={() => setAttendancePickerState({ rect: document.querySelector(`[data-student="${student.id}"][data-date="${d}"]`)?.getBoundingClientRect() || new DOMRect(), studentId: student.id, date: d })}
                            className="w-7 h-7 rounded-md border-2 border-dashed border-gray-300 hover:border-gray-400 hover:bg-gray-100 text-gray-400 text-xs transition-colors"
                          >+</button>
                        )}
                      </td>
                    );
