        ...
        {selectedStudentId && (
          <div className="px-6 py-4 border-b border-gray-100 space-y-4">
            ...
            {currentAttendance ? (
              ...
              /* Блок для создания новой отметки, когда её ещё нет */
              <div className="space-y-3">
                <div className="p-3 bg-blue-50 rounded-xl border border-blue-200">
                  <p className="text-sm font-medium text-blue-800">Новая отметка</p>
                  <p className="text-xs text-blue-600 mt-1">Выберите тип отметки и уроки ниже</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-700 mb-2">Выберите тип отметки:</p>
                  <div className="flex flex-wrap gap-2">
                    {ATTENDANCE_TYPES.map(type => (
                      <button
                        key={type.value}
  ...
  if (lessonPageDate) {
    ...
    return (
      <div className="animate-fadeIn space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <select value={lpLessonType?.type || ''} onChange={e => {
                ...
                {customLessonTypes && Array.isArray(customLessonTypes) && customLessonTypes.map(lt => (
                  <option key={lt.id} value={lt.value}>{lt.label}</option>
                ))}
              </select>
            </div>
          </div>
        ...
        {/* Секция оценок и посещаемости */}
        <div className="mt-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Оценки и посещаемость</h3>
          {/* Таблица оценок */}
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-4 py-3 text-center text-sm font-semibold text-gray-700">Оценка</th>
                  <th className="px-4 py-3 text-center text-sm font-semibold text-gray-700">Посещаемость</th>
                </tr>
              </thead>
              <tbody>
                {lpStudentGrades.map(student => {
                  const grade = getGrade(student.id, lessonPageDate, undefined, lessonPageLessonNum);
                  const attendance = getAttendanceMark(student.id, lessonPageDate);
                  return (
                    <tr key={student.id} className="border-t border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        {grade ? (
                          <GradeWithTooltip value={grade.value} />
                        ) : (
                          <button
                            onClick={(e) => {
                              const rect = (e.target as HTMLElement).getBoundingClientRect();
                              setGradePickerState({ rect, studentId: student.id, date: lessonPageDate, lessonNumber: lessonPageLessonNum });
                            }}
                      ...
                      <td className="px-4 py-3 text-center">
                        {attendance ? (
                          <button
                            onClick={(e) => {
                              const rect = (e.target as HTMLElement).getBoundingClientRect();
                              setAttendancePickerState({ rect, studentId: student.id, date: lessonPageDate });
                            }}
                            className={`px-3 py-1.5 rounded-lg text-sm font-bold ${ATTENDANCE_TYPES.find(t => t.value === attendance.type)?.bgColor} ${ATTENDANCE_TYPES.find(t => t.value === attendance.type)?.color}`}
                          >
                            {ATTENDANCE_TYPES.find(t => t.value === attendance.type)?.short}
                          </button>
                        ) : (
                          <button
                            onClick={(e) => {
...
export { AttendanceCalendar, AttendanceModal, AttendancePickerPortal, GradePickerPortal, Journal, LessonTypesManager, AdminDashboard, AdminSettingsModal, GradeWithTooltip };