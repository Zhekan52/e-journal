...
export const QuestionEditor: React.FC<QuestionEditorProps> = ({
  ...
  const [showFormula, setShowFormula] = useState(!!formula);

  // Автоматически включаем флажок если есть изображение
  useEffect(() => {
    if (image) setShowImage(true);
  }, [image]);

  // Автоматически включаем флажок если есть формула
  useEffect(() => {
    if (formula) setShowFormula(true);
  }, [formula]);
  ...
  const toggleFormula = (checked: boolean) => {
    setShowFormula(checked);
    if (!checked) {
      onFormulaChange('');
    }
  };
      ...
      <div>
        <label className="flex items-center gap-3 cursor-pointer mb-2">
          <input
            type="checkbox"
            checked={showFormula}
            onChange={(e) => toggleFormula(e.target.checked)}
            className="w-5 h-5 text-primary-600 rounded focus:ring-primary-500"
          />
          <span className="text-sm font-medium text-gray-700">Добавить формулу (LaTeX)</span>
        </label>
